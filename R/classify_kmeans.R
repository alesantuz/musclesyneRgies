...
classify_kmeans <- function(x,
  ...
  orders <- data.frame(clusters_P=clust_P$cluster,
  ...
  if (isTRUE(interactive)) {
    ...
    red_factor <- 25
    graphics::par(mfrow=c(clust_num, 2),
  ...
  if (!is.na(path_for_graphs)) {
    ...
    Cairo::Cairo(file=paste0(path_for_graphs, "SYNS_k-means_classification_clusters.", filetype),
  ...
  for (ss in 1:clust_num) {
    syn_perc[ss] <- round(length(grep(paste0("^", ss, "$"),
                                      orders_new$syn_classified))/length(trials)*100, 0)
